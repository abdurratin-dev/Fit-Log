...
export default function MyPlanSelectedCard({
  ...
  planSort: IWorkOutDataType[];
}) {
  if(planSort.length <= 0 ){
    return(
      <>
      <div className="mt-5 flex flex-col justify-center items-center h-100 gap-4 border-2 border-dashed rounded-3xl border-gray-700">
        ...
        <p className="text-[#A1A1AA] sm:text-md text-sm text-center">Browse the library and add a lift to get today moving.</p>
  ...
  return (
    <div className="mt-5 grid gap-5">
      {planSort.map((workOut: IWorkOutDataType, index: number) => {
        return (
